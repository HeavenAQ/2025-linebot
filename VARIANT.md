...
| Web app | `SkillSummary.tsx`, `useSkillSummary.ts`, the `gpt-chat` page and its menu entry; the AI summary on the personal page; the chat-history panel in `WeeklyReview.tsx`; the coaching-cue markers, cue list, pause intervals and pause legend in `VideoComparison.tsx`, whose captions show only the current technical checkpoint, and with them `main`'s 教練建議版／分析原片 switch, which names a render this branch never produces; `CoachingCue` in the schemas |
