...
from badminton_analysis.ml.clear_feedback import sample_video_frames, prompt_context, system_instructions
...
def eg27_evidence():
    bounds = [(51, 81, 57), (84, 100, 95), (94, 112, 112),
              (114, 121, 119), (119, 123, 119), (126, 128, 128)]
    intervals = {rule.id: dict(start=a, end=b, anchor=c)
                 for rule, (a, b, c) in zip(get_skill_spec('smash').rules, bounds)}
    return build_checkpoint_evidence(
        intervals=intervals, window_start=60, window_end=130,
        initial_frame=60, balance_anchor=112, selected_endpoint=130,
        balance_measurement=dict(available=True, event_start=103, event_end=107,
                                 handedness='right', sustained_gap=.568))
...
def samples(tmp_path):
    ...
    writer.release()
    return sample_video_frames(video, tmp_path / 'frames',
        phase_indices=(0, 27, 53, 59, 63),
        source_frame_indices=[round(i * 70 / 63) for i in range(64)],
        spec=get_skill_spec('smash'), checkpoint_evidence=eg27_evidence())
...
def test_exact_early_balance_frames_are_supplied(samples):
    ...
    for sample in owned:
        assert abs(cv2.imread(str(sample.image_path)).mean() - 3 * sample.source_frame_index) < 3
...
def grade():
    return dict(total_score=96.5, checkpoint_evidence=eg27_evidence(),
        criteria=[dict(rule_reference=r.id, maximum=r.maximum,
                       score=1.5 if r.id == 'arm_balance' else r.maximum)
                  for r in get_skill_spec('smash').rules])


def analysis(frame):
    return dict(skill='smash', language='zh-TW', overall_feedback='請留意蓄力初段持拍手的位置。',
        problems=[dict(priority='高', title='雙手手肘平衡',
            feedback='非慣用手抬起後，持拍手也要及時抬起。',
            evidence='連續數幀可見慣用手仍然低於自身肩部。', frame_index=frame,
            phase='rotation', joint_ids=[8, 10], rule_reference='arm_balance', confidence=.9)])
...
def test_valid_evidence_survives_without_anchor_snapping(samples):
    ...
    result = CoachingGenerator._normalize_analysis(analysis(sample.frame_index),
        spec=get_skill_spec('smash'), correction_grade=grade(),
        phase_indices=(0, 27, 53, 59, 63), samples=samples)
    ...
    with pytest.raises(ValueError, match='scored evidence'):
        CoachingGenerator._normalize_analysis(analysis(27), spec=get_skill_spec('smash'),
            correction_grade=grade(), phase_indices=(0, 27, 53, 59, 63), samples=samples)


def test_rubric_and_prompt_agree(samples):
    spec = get_skill_spec('smash')
    assert [r.maximum for r in spec.rules] == [5, 20, 5, 20, 30, 20]
    context = prompt_context({}, samples, phase_indices=(0, 27, 53, 59, 63),
                             correction_grade=grade(), spec=spec)
