...
> **This is the `variant/no-llm` branch.** It is the same product with every
> large-language-model layer removed, so it can be run beside the original and
> compared. It keeps all of the non-LLM AI -- pose estimation, the diffusion
> correction, grading, checkpoints and expert matching. `badminton_analysis_ai/`
> and `proto/` are identical to `main`: the variant shares the deployed analysis
> service and turns coaching off per deployment with
> `ANALYSIS_SKIP_COACHING=true`, which makes the service branch before the only
> stage that sends learner frames to a third party. Everything below describes
> `main`; where it mentions GPT chat, summaries, the weekly preview push or
> coaching cues, those do not exist on this branch. What differs for the
> variant, and how to stand it up, is in [VARIANT.md](VARIANT.md).
>
