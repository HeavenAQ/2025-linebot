...
func (app *App) handleUserState(event *linebot.Event, user *db.UserData, session *db.UserSession, replyToken string) {
	...
	// 3. A typed message carries no postback payload. A step that is waiting
...
func awaitingButtonTap(session *db.UserSession) bool {
	...
	case db.ViewingPortfoilo:
		return true
	case db.WritingPreviewNote, db.WritingReflectionNote:
		return session.ActionStep == db.SelectingSkill || session.ActionStep == db.SelectingPortfolio
	case db.ViewingExpertVideos, db.AnalyzingVideo:
