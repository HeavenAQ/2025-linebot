...
func TestAwaitingButtonTapCoversTheStepsThatParseAPostback(t *testing.T) {
	...
	for _, session := range []db.UserSession{
		...
		{UserState: db.WritingNotes, ActionStep: db.SelectingSkill},
		{UserState: db.WritingNotes, ActionStep: db.SelectingPortfolio},
		{UserState: db.ChattingWithGPT, ActionStep: db.SelectingSkill},
...
func TestAwaitingButtonTapLeavesTheTypingStepsAlone(t *testing.T) {
	...
	for _, session := range []db.UserSession{
		{UserState: db.ChattingWithGPT, ActionStep: db.Chatting},
		{UserState: db.WritingNotes, ActionStep: db.WritingReflection},
