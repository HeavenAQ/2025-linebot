...
// 1. Create a conversation (thread) once and store its ID.
// 2. For each user message, call Responses.New with the conversation ID and model.
...
func (client *Client) CreateThread() (*conversations.Conversation, error) {
	threadReq := conversations.ConversationNewParams{
		Items:    []responses.ResponseInputItemUnionParam{},
		Metadata: shared.Metadata{},
	}

	conversation, err := client.Client.Conversations.New(*client.Ctx, threadReq)
	if err != nil {
...
func (client *Client) RetrieveThread(conversationID string) (*conversations.Conversation, error) {
...
// AddMessageToThread sends a user message via Responses API
// and returns the assistant's generated text output.
func (client *Client) AddMessageToThread(conversationID, message string) (string, error) {
