...
> compared. `badminton_analysis_ai/` and `proto/` are identical to `main`: the
> variant shares the deployed analysis service and turns coaching off per
> deployment with `ANALYSIS_SKIP_COACHING=true`, which makes the service branch
> before the only stage that sends learner frames to a third party. Everything
> below describes that shared service; what differs for the variant, and how to
> stand it up, is in [VARIANT.md](VARIANT.md).
>
