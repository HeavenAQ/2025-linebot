...
func (app *App) handleTextMessage(event *linebot.Event, message *linebot.TextMessage, user *db.UserData, session *db.UserSession) {
	...
	if err != nil {
		app.Logger.Info.Println("Incoming text is not a rich menu command; routing by session state")
...
func (app *App) handleRichMenuMessage(
	...
	case db.WritingPreviewNote:
		app.processWritingPreviewNote(user, replyToken)
	case db.WritingReflectionNote:
		app.processWritingReflectionNote(user, replyToken)
	default:
