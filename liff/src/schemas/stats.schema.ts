...
const BasePointVariants = z.union([
  ...
  z
    ...
    .transform(v => ({ date: v.date, value: v.value })),
  // alternate value keys
  z.object({ date: z.string(), avg: z.number() }).transform(v => ({ date: v.date, value: v.avg })),
  z
    .object({ date: z.string(), score: z.number() })
    .transform(v => ({ date: v.date, value: v.score })),
  // alternate date keys
  z
    .object({ day: z.string(), value: z.number() })
    .transform(v => ({ date: v.day, value: v.value })),
  ...
  z
    .object({ day: z.string(), score: z.number() })
    .transform(v => ({ date: v.day, value: v.score })),
